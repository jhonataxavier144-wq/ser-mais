...
export default function NewCoursePage() {
  ...
  const [course, setCourse] = useState({
    ...
    quizUrl: "",
    modules: [
      { id: generateId(), title: "Módulo 1", lessons: [{ id: generateId(), title: "Aula 1", videoUrl: "" }] }
    ]
  });
  ...
  const addModule = () => {
    setCourse({
      ...course,
      modules: [...course.modules, { id: generateId(), title: `Módulo ${course.modules.length + 1}`, lessons: [{ id: generateId(), title: "Aula 1", videoUrl: "" }] }]
    });
  };
  ...
  const addLesson = (mIdx: number) => {
    const newModules = [...course.modules];
    newModules[mIdx].lessons.push({ id: generateId(), title: `Aula ${newModules[mIdx].lessons.length + 1}`, videoUrl: "" });
    setCourse({ ...course, modules: newModules });
  };
  ...
  const handleVideoUpload = async (mIdx: number, lIdx: number, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      updateLesson(mIdx, lIdx, "videoUrl", downloadURL);
    } catch (err) {
      console.error(err);
      alert("Erro no upload do vídeo para o Firebase.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ background: 'var(--off-white)', minHeight: '100vh', padding: '40px' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '28px', fontWeight: 800, marginBottom: '32px' }}>Criar Novo Curso</h1>
        ...
        <form onSubmit={handleSubmit}>
          <div className="card" style={{ marginBottom: '24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <div style={{ 
                  ...
                  <label style={{ position: 'absolute', inset: 0, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.3)', color: 'white', opacity: course.thumbnail ? 0 : 1, transition: 'opacity 0.2s' }}>
                    <span style={{ fontSize: '12px', fontWeight: 600 }}>{course.thumbnail ? "Alterar" : "Upload"}</span>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    ...
                    <input 
                      ...
                      required 
                      placeholder="Ex: Dominando o Excel"
            ...
            <div style={{ borderTop: '1px solid var(--gray-light)', paddingTop: '24px', marginTop: '8px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label style={{ fontSize: '13px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <Brain size={16} /> Link do Quiz Final
                </label>
                <input 
                  type="text"
          ...
          <div style={{ display: 'flex', gap: '16px', marginTop: '32px' }}>
            ...
            >
              <Save size={18} /> {loading ? "Processando..." : "Salvar Curso"}
            </button>
          </div>
        </form>
      </div>
