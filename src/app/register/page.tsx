...
export default function RegisterPage() {
  ...
  const passwordStrength = password.length >= 8 ? 3 : password.length >= 6 ? 2 : password.length > 0 ? 1 : 0;
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', width: '400px', height: '400px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(0,156,59,0.15), transparent)', bottom: '-100px', left: '-100px' }}></div>
        <div style={{ position: 'absolute', width: '300px', height: '300px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(0,156,59,0.1), transparent)', top: '-80px', right: '-80px' }}></div>
        ...
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', maxWidth: '380px' }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '26px', fontWeight: 700, color: 'white', marginBottom: '16px', lineHeight: 1.2 }}>Comece sua jornada gratuita hoje</h2>
          <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.45)', lineHeight: 1.7 }}>Cadastre-se e tenha acesso a todos os cursos e certificados sem custo algum.</p>
          
          <div style={{ marginTop: '40px', display: 'flex', flexDirection: 'column', gap: '14px', textAlign: 'left' }}>
            {["Acesso a todos os cursos grátis", "Certificados de conclusão", "Gamificação e conquistas", "Progresso salvo na nuvem"].map(item => (
              <div key={item} style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'rgba(255,255,255,0.6)', fontSize: '13px' }}>
                <div style={{ width: '20px', height: '20px', borderRadius: '50%', background: 'rgba(0,156,59,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px', flex: 1, minWidth: '320px' }}>
        <div style={{ width: '100%', maxWidth: '400px' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '26px', fontWeight: 800, color: 'var(--black)', marginBottom: '6px' }}>Criar conta grátis 🌱</h1>
          <p style={{ fontSize: '14px', color: 'var(--gray)', marginBottom: '32px' }}>Preencha seus dados para começar a aprender.</p>
          ...
          <form onSubmit={handleRegister}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginBottom: '14px' }}>
              <label style={{ fontSize: '13px', fontWeight: 500, color: 'var(--gray-dark)' }}>Nome completo</label>
              <input 
                type="text" 
                value={name}
            ...
            {/* Password strength */}
            {password.length > 0 && (
              <div style={{ display: 'flex', gap: '4px', marginBottom: '14px', marginTop: '6px' }}>
                {[1,2,3].map(i => (
                  <div key={i} style={{ flex: 1, height: '3px', borderRadius: '2px', background: passwordStrength >= i ? (passwordStrength === 1 ? '#EF4444' : passwordStrength === 2 ? '#F59E0B' : 'var(--green)') : 'var(--gray-light)', transition: 'background 0.3s' }}></div>
                ))}
                <span style={{ fontSize: '10px', color: passwordStrength === 1 ? '#EF4444' : passwordStrength === 2 ? '#F59E0B' : 'var(--green)', fontWeight: 600, marginLeft: '6px' }}>
          ...
          <div style={{ textAlign: 'center', marginTop: '24px', fontSize: '14px', color: 'var(--gray)' }}>
            Já tem conta? <Link href="/login" style={{ color: 'var(--green)', fontWeight: 600, textDecoration: 'none' }}>Entrar</Link>
          </div>

          <p style={{ fontSize: '11px', color: 'var(--gray)', textAlign: 'center', marginTop: '24px', lineHeight: 1.6, opacity: 0.7 }}>
            Ao se cadastrar, você concorda com nossos <Link href="#" style={{ color: 'var(--green)' }}>Termos de Uso</Link> e <Link href="#" style={{ color: 'var(--green)' }}>Política de Privacidade</Link>.
          </p>
        </div>
      </div>
